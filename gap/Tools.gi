...
InstallMethod( IdealBasisOverCoefficientRing,
  ...
  function( G )
    ...
    W := List( [ 1 .. NrRows( G ) ], i -> BasisCoefficientsOfRingElement( G[ i, 1 ] / R ) );
...
InstallMethod( IdealBasisToGroebner, 
  ...
  function( M )
    ...
    C := List( [ 1 .. NrRows( M ) ], i -> BasisCoefficientsOfRingElement( M[ i, 1 ] ) );
    ...
    C := List( [ 1 .. NrRows( GI ) ], i -> Coefficients( GI[ i, 1 ] )!.monomials[1] );
    
    for i in [ 1 .. NrRows( GI ) ] do
        
        if not IsZero( C[i] / S ) then
            
            lambda := BasisCoefficientsOfRingElement( ( GI[ i, 1 ] - C[i] ) / R  );
  ...
  function( f )
    ...
    for i in [ 1 .. NrRows( coeffs ) ] do
        ...
        if not IsOne( monoms[i] ) then
            ...
            f := f + coeffs[ i, 1 ] * Degree( monoms[i] ) * indets[1]^( Degree( monoms[i] ) - 1 );
...
InstallMethod( GeneratorOfAnElementNotContainedInAnyHyperplane,
  ...
  function( L )
    ...
    while true do
        ...
        Perform( [ 1 .. NrColumns( L ) ], function(j) lambda[ 1, j  ] := Random( rs, [ - 50 .. 50 ] ) / R; end );
             ...
             if not IsZero( a / S ) then
                ...
                if not IsZero( DecideZeroRows( b, BB ) ) then
                    ...
                    while IsZero( i ) and ( (l - 1) < NrRows( syz ) ) do
                        if not IsZero( syz[ l, NrColumns( syz ) ] ) then
                    ...
                    if IsZero( syz[ 1, NrColumns( syz ) ] ) then
                        
                        Error();
                   
                    #elif not IsOne( syz[ 1, NrColumns( syz ) ] ) then
                    ## if not IsOne( syz[ 1, NrColumns( syz ) ] ) then the
